-- Fix settle_mycroft_shadow_ai para mercados "Próximo Gol: <Time>"
-- Bug: a versão antiga marcava GREEN sempre que houvesse qualquer gol após a entrada,
-- ignorando QUAL time marcou. Agora extraímos o nome do time do label do mercado
-- e comparamos com home_team/away_team (snapshot) para decidir GREEN/RED.

CREATE OR REPLACE FUNCTION public.settle_mycroft_shadow_ai(
  p_id uuid,
  p_score_home integer,
  p_score_away integer,
  p_reason text DEFAULT 'auto'::text
)
RETURNS text
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
AS $function$
DECLARE
  v_market TEXT;
  v_verdict TEXT;
  v_home TEXT;
  v_away TEXT;
  v_total INT := COALESCE(p_score_home,0) + COALESCE(p_score_away,0);
  v_appr_total INT;
  v_ah INT;
  v_aa INT;
  v_result TEXT;
  v_line NUMERIC;
  v_team_label TEXT;
  v_team_norm TEXT;
  v_home_norm TEXT;
  v_away_norm TEXT;
  v_home_scored BOOLEAN;
  v_away_scored BOOLEAN;
BEGIN
  SELECT market, verdict, approved_at_score_home, approved_at_score_away, home_team, away_team
    INTO v_market, v_verdict, v_ah, v_aa, v_home, v_away
  FROM public.mycroft_analyses_shadow_ai WHERE id = p_id;
  IF v_market IS NULL THEN RETURN 'not_found'; END IF;
  IF v_verdict NOT IN ('APROVADO','APROVADO_SITUACIONAL','LABAREDA') THEN RETURN 'not_active'; END IF;

  IF v_market ~* '(pr[óo]ximo\s+gol|next\s+goal)' THEN
    IF v_ah IS NULL OR v_aa IS NULL THEN RETURN 'missing_snapshot'; END IF;
    v_appr_total := COALESCE(v_ah,0) + COALESCE(v_aa,0);
    v_home_scored := COALESCE(p_score_home,0) > COALESCE(v_ah,0);
    v_away_scored := COALESCE(p_score_away,0) > COALESCE(v_aa,0);

    -- Tenta extrair time alvo do label: "Próximo Gol: <time>" / "Próximo Gol (<time>)" / "Próximo Gol - <time>"
    v_team_label := NULL;
    v_team_label := (regexp_matches(v_market, '(?:pr[óo]ximo\s+gol|next\s+goal)\s*[:\-\(]\s*([^)]+)\)?', 'i'))[1];

    IF v_team_label IS NULL OR btrim(v_team_label) = '' THEN
      -- Mercado genérico "Próximo Gol" / "Próximo Gol Sim": GREEN se houve qualquer gol após entrada
      IF v_market ~* 'n[ãa]o' THEN
        v_result := CASE WHEN v_total > v_appr_total THEN 'red' ELSE 'green' END;
      ELSE
        v_result := CASE WHEN v_total > v_appr_total THEN 'green' ELSE 'red' END;
      END IF;
    ELSE
      -- Mercado com time específico: precisa que ESSE time marque o próximo gol
      v_team_norm := lower(unaccent(btrim(v_team_label)));
      v_home_norm := lower(unaccent(COALESCE(v_home,'')));
      v_away_norm := lower(unaccent(COALESCE(v_away,'')));

      IF v_home_norm <> '' AND (v_team_norm = v_home_norm OR position(v_team_norm in v_home_norm) > 0 OR position(v_home_norm in v_team_norm) > 0) THEN
        -- Aposta na casa
        IF v_home_scored AND NOT v_away_scored THEN
          v_result := 'green';
        ELSIF v_away_scored AND NOT v_home_scored THEN
          v_result := 'red';
        ELSIF v_home_scored AND v_away_scored THEN
          -- Ambos marcaram após entrada: não temos ordem, mantemos conservador como RED
          v_result := 'red';
        ELSE
          RETURN 'no_goal_yet';
        END IF;
      ELSIF v_away_norm <> '' AND (v_team_norm = v_away_norm OR position(v_team_norm in v_away_norm) > 0 OR position(v_away_norm in v_team_norm) > 0) THEN
        -- Aposta no visitante
        IF v_away_scored AND NOT v_home_scored THEN
          v_result := 'green';
        ELSIF v_home_scored AND NOT v_away_scored THEN
          v_result := 'red';
        ELSIF v_home_scored AND v_away_scored THEN
          v_result := 'red';
        ELSE
          RETURN 'no_goal_yet';
        END IF;
      ELSE
        -- Não conseguiu casar o time -> conservador
        RETURN 'unmatched_team';
      END IF;
    END IF;
  ELSIF v_market ~* 'over\s*([0-9]+(\.[0-9]+)?)' THEN
    v_line := (regexp_matches(v_market, 'over\s*([0-9]+(\.[0-9]+)?)', 'i'))[1]::numeric;
    v_result := CASE WHEN v_total > v_line THEN 'green' ELSE 'red' END;
  ELSIF v_market ~* 'under\s*([0-9]+(\.[0-9]+)?)' THEN
    v_line := (regexp_matches(v_market, 'under\s*([0-9]+(\.[0-9]+)?)', 'i'))[1]::numeric;
    v_result := CASE WHEN v_total < v_line THEN 'green' ELSE 'red' END;
  ELSIF v_market ~* '(btts|ambas\s+marcam|both\s+teams\s+to\s+score).*sim|^sim' THEN
    v_result := CASE WHEN p_score_home > 0 AND p_score_away > 0 THEN 'green' ELSE 'red' END;
  ELSIF v_market ~* '(btts|ambas|both).*n[ãa]o' THEN
    v_result := CASE WHEN p_score_home = 0 OR p_score_away = 0 THEN 'green' ELSE 'red' END;
  ELSE
    RETURN 'unsupported_market';
  END IF;

  UPDATE public.mycroft_analyses_shadow_ai
  SET result = v_result,
      final_score_home = p_score_home,
      final_score_away = p_score_away,
      settled_at = now(),
      settle_reason = p_reason
  WHERE id = p_id;

  RETURN v_result;
END;
$function$;

-- Garante extensão unaccent (idempotente)
CREATE EXTENSION IF NOT EXISTS unaccent;

-- Backfill: re-liquida TODOS os "Próximo Gol: <time>" já liquidados em GREEN/RED
-- usando o placar final salvo, para corrigir liquidações erradas históricas.
DO $$
DECLARE
  r RECORD;
BEGIN
  FOR r IN
    SELECT id, final_score_home, final_score_away
    FROM public.mycroft_analyses_shadow_ai
    WHERE verdict IN ('APROVADO','APROVADO_SITUACIONAL','LABAREDA')
      AND market ~* '(pr[óo]ximo\s+gol|next\s+goal)'
      AND result IN ('green','red')
      AND final_score_home IS NOT NULL
      AND final_score_away IS NOT NULL
  LOOP
    -- Limpa para permitir re-liquidação
    UPDATE public.mycroft_analyses_shadow_ai
       SET result = NULL, settled_at = NULL, settle_reason = NULL
     WHERE id = r.id;
    PERFORM public.settle_mycroft_shadow_ai(
      r.id, r.final_score_home, r.final_score_away, 'backfill_team_aware_fix'
    );
  END LOOP;
END $$;