...
-- 1) Trigger BC por faixa de odd (tabela oficial)
...
BEGIN
  ...
  -- BC base por faixa de odd (tabela oficial)
  v_odd := COALESCE(NEW.odd, 0);
  v_base := CASE
    WHEN v_odd >= 4.00 THEN 70
    WHEN v_odd >= 3.00 THEN 50
    WHEN v_odd >= 2.30 THEN 35
    WHEN v_odd >= 1.90 THEN 25
    WHEN v_odd >= 1.60 THEN 15
    WHEN v_odd >= 1.30 THEN 10
    ELSE 10
  END;

  -- Pequeno bônus por lucro proporcional (cap 30 BC) — mantém incentivo extra
  IF NEW.stake IS NOT NULL AND NEW.stake > 0 AND NEW.profit_loss IS NOT NULL AND NEW.profit_loss > 0 THEN
    v_bonus := LEAST(30, FLOOR((NEW.profit_loss / NEW.stake) * 10)::INT);
  END IF;
...
-- 2) View leaderboard com Hórus forçado em rank=1
...
SELECT
  ...
  plan, plan_active,
  -- Hórus sempre primeiro: ordena por (is_horus DESC, roi DESC)
  RANK() OVER (ORDER BY is_horus DESC, roi_pct DESC, total_bets DESC) AS rank
FROM combined;
