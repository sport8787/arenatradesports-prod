-- ============================================================
-- Exclui Under 2.5 do cálculo de ROI (2026-06-07)
-- Under 2.5 é estratégia de cash-out (saída rápida em alguns ticks),
-- não uma entrada de resultado. Contamina o ROI com REDs que não
-- representam a estratégia principal.
-- ============================================================
...
BEGIN
  ...
  SELECT jsonb_build_object(
    ...
    'greens',         count(*) filter (where result = 'GREEN'
                        AND market NOT ILIKE '%under%2.5%'),
    'reds',           count(*) filter (where result = 'RED'
                        AND market NOT ILIKE '%under%2.5%'),
    'voids',          count(*) filter (where result in ('VOID', 'HALF_GREEN', 'HALF_RED')),
    'win_rate',       round(
                        (count(*) filter (where result = 'GREEN' AND market NOT ILIKE '%under%2.5%')::numeric
                        / nullif(count(*) filter (where result in ('GREEN', 'RED') AND market NOT ILIKE '%under%2.5%'), 0)
                        ) * 100, 1),
    'roi_percent',    round(
                        (coalesce(sum(profit_loss) filter (where result in ('GREEN', 'RED') AND market NOT ILIKE '%under%2.5%'), 0)
                        / nullif(sum(stake) filter (where result in ('GREEN', 'RED') AND market NOT ILIKE '%under%2.5%'), 0)
                        ) * 100, 2),
    'profit_total',   round(coalesce(sum(profit_loss) filter (where result in ('GREEN', 'RED') AND market NOT ILIKE '%under%2.5%'), 0), 2),
    'stake_total',    round(coalesce(sum(stake)       filter (where result in ('GREEN', 'RED') AND market NOT ILIKE '%under%2.5%'), 0), 2)
  )
  INTO v_summary
