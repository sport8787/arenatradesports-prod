...
function applyMarketSanityVeto(params: {
  ...
  if (isOver05HT) {
    if (minute < 5 || minute > 30 || totalGoals >= 1) {
      vetoReason = `Over 0.5 HT bloqueado: minuto ${minute}, placar ${sh}x${sa}. Janela válida: minuto 5–30 do 1º tempo e placar 0x0.`;
    }
  }
    ...
    for (const m of liveMatches) {
      ...
      try {
        ...
        await sb.from("mycroft_analyses_shadow_ai").insert({
          ...
          provider: aiRes.provider === "deepseek" ? "deepseek-ai" : "groq-ai",
          model: aiRes.model,
          approved_at_minute: minute,
          approved_at_score_home: m.score_home ?? null,
          approved_at_score_away: m.score_away ?? null,
          stats_snapshot: isApproved ? { provider: aiRes.provider, model: aiRes.model, minute, score_home: m.score_home ?? 0, score_away: m.score_away ?? 0, home_team: m.home_team, away_team: m.away_team, championship: m.championship, stats: m.stats || {} } : null,
          latency_ms: aiRes.ms,
