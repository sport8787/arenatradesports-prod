...
Forneça o relatório forense completo em JSON puro (sem markdown code blocks).`;
    ...
    // Telegram desativado a pedido do usuário (grupo principal estava poluído com sinais ao vivo).
    // Será reativado em grupo dedicado de sinais ao vivo no futuro.
    console.log(`[ArenaTraderAnalyze] 🔕 Telegram disabled — analysis stored only (${asset.symbol} | ${parsed.status_mercado})`);
